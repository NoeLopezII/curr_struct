...
class OpenAIService {
  static final _apiKey = dotenv.get('OPENAI_API_KEY');
  ...
  static Future<int> scoreExplanation(String text, Explanation explanation) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (scoreMatch != null) {
        return int.parse(scoreMatch.group(0)!);
      }
      return 0;
    }
    return 0;
  ...
  static Future<Description> generateTeaching(String ocrText) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://api.openai.com/v1/chat/completions'), // Same URL structure
        headers: {
          'Authorization': 'Bearer $_apiKey', // Same header structure
          'Content-Type': 'application/json',
        },
        body: jsonEncode({
          'model': 'o3-mini', // Using chat model, similar to checkConsensus
      ...
      if (response.statusCode == 200) {
        ...
        return Description(
          modelName: 'Openai', // Identify the source model
          teaching: content.trim(), // Trim whitespace
        );
      } else {
        // Similar error handling approach: log and return an error Description
        print('Openai generateTeaching Error ${response.statusCode}: ${response.body}');
        return Description(
            modelName: 'Openai',
            teaching: 'Error: Could not generate teaching from Openai.');
      }
    } catch (e) {
      // Catch potential network or other errors during the request
      print('Openai generateTeaching Request Error: $e');
      return Description(
          modelName: 'Openai',
          teaching: 'Error: Failed to request teaching from Openai.');
    }
  }
  ...
  static Future<int> scoreTeaching(String ocrText, Description teachingToScore) async {
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'model': 'o3-mini', // Using reasoner model, similar to scoreExplanation
          'messages': [{'role': 'user', 'content': prompt}],
        }),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (scoreMatch != null) {
          ...
          // Clamp score to the required 1-10 range
          if (score < 1) return 1;
          if (score > 100) return 100;
          return score;
        }
        // Similar fallback if parsing fails
        print("Openai scoreTeaching - Could not parse score from: $content");
        return 1; // Return lowest score in the desired range (1-10)
      } else {
        // Similar error handling: log and return default score
        print('Openai scoreTeaching Error ${response.statusCode}: ${response.body}');
        return 1; // Return lowest score in range on API error
      }
    } catch (e) {
      // Catch potential network or other errors during the request
      print('Openai scoreTeaching Request Error: $e');
